...
// Monta ranking, histórico e gráficos
function montar(dados){
  ...
  const escolasMap = {};
  let somaIPT = 0, critica=0, atencaoCount=0;

  dados.forEach(d => {
    let peso=1, classe="adequada";
    const s = (d.status||"").toLowerCase();
    if(s.includes("crít")){ peso=4; classe="critica"; critica++; statusCount.critica++; }
    else if(s.includes("aten")){ peso=3; classe="atencao"; atencaoCount++; statusCount.atencao++; }
    ...
    const rt = Number(d.rt||0); // Frequência no status
    // IPT com RT ponderado
  ...
  kpiAtencao.innerText = atencaoCount;
